...
3和4，解法是混淆流量，shadowsocks是一个合格的解（并不是最简）。但是，邪恶还没有完：

5. tcp连接建立后，随机被拦端口，后续流量无法到达vps，同时，老端口恢复
6. udp随机被拦端口，同时，老端口恢复
...
3. 混淆域名
4. 混淆流量（最简）
5. tcp可变端口，不ssl，与远端建立连接后乒乓一次，两秒没收到乓，换一个端口连
6. udp可变端口，不心跳
7. 气死
