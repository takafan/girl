...
原本需依赖tcp的重传到达目的地/返回，换一条通畅线路，延迟可以降低到和ping包一致。

体现到游戏上，读条和转圈变快。

体现到下载上，下载速度从几K，几十K，变成几M，甚至几十M。

还没完，妹子来了，和传统代理稍不同，妹子是一根通道：
...
udp的快，主要指低延迟，从家到代理服务器的ping值，加上代理服务器到目的地的ping值，小于直连，代理才有意义。追求低延迟，udp很需要区分国内外。目的地在国外，可以给它找条更短的线路，目的地在国内，直连就是最快的。

但udp不能区分国内外，因为p2p的存在。要是把国内ip段设为直连，采用p2p联机的游戏会怎么样：游戏匹配服务器在海外，匹配到国内玩家，他从他洞里出来穿你（映射到远端的）洞，通。你却不走打好的洞突然直连穿他，不通。

并且现实中，udp代理可以说不存在。ios，android，ps4，switch，自带http代理，http代理不支持udp。windows和macos可设socks5，但走不走代理，还是由应用程序实现，实现socks5，有，实现socks5中的UDP ASSOCIATE部分，没见过。socks5支持udp，但形同虚设。
...
用udp来中转，udp邪恶没管，无需改动流量。但udp会遇到2个问题：
