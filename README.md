...
面对出国线路限速式的掉包，传输协议唯一能做的就是重传。tcp的重传策略是超时主动重传，一系列补丁（也叫拥控算法）中，SACK是真正起到作用的一个补丁，在SACK之前，tcp只会单发确认。SACK让tcp得以跳跃的确认收到的包。但40个字节放不下几个头尾，只要断的稍微碎一点，不那么连续，SACK就不够用了。妹子的重传策略我称之为被动重传，下限极高，一秒钟掉一千个包可以把tcp的速度打成一折，0.1折，但掉在妹子身上只是做减法。
