...
5. 一旦目的地ip被收集，任何tcp建立连接后来源端口被封，后续流量无法到达
6. 一旦目的地ip被收集，任何udp第一段流量后来源端口被封，后续流量无法到达

邪恶很聪明的封来源，而不是封目的地，防止你利用它。
...
3. 混淆域名（可以自定义）
4. 混淆流量（可以自定义）
5. tcp不停的建https连接或者socks5连接（包括shadowsocks）即可提高被收集几率。而不使用这两个协议，则没发现被收集的情况。
