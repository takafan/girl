...
做到了。但还不够。妹子来了，和传统代理稍不同，妹子是一根通道：
...
面对出国线路限速式的掉包，传输协议唯一能做的就是重传。tcp的SACK是真正起到作用的一个补丁，在这个补丁之前，tcp只会单发确认。SACK让tcp学会了跳跃的确认收到的包。但40个字节放不下几个头尾，只要断的稍微碎一点，不那么连续，SACK就不够用了。相对于tcp的超时主动重传，妹子的重传策略我称之为被动重传，效率远超tcp以及任何一个tcp大补丁（也叫拥控算法）。
...
妹子给流量打上序号，打包成udp，在应用层传输。一边收，一边半秒被告知一次对面的状态，例如，收到1-5，和跳号的8，这半秒被告知发到10，得出6-7，9-10两个号码段，要求对面重传。简称被动重传。
...
换一台掉包厉害的，电信单程cn2美国服务器，hybla降到80K，bbr降到10K，cubic降到4K，在tcp全体投降的状态下，妹子依然8M，比hybla快100倍，比bbr快800倍，比cubic快2000倍。
